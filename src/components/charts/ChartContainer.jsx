        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {data.trends[selectedCommodity]?.map(
            ({ clusterId, data: priceData }) => {
              ...
              const trendPercent = ((trend / priceData[0]) * 100).toFixed(1);
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex flex-col items-center justify-center h-96">
            ...
            <p className="text-gray-600 text-center mb-4 max-w-md">
              Lakukan analisis untuk melihat visualisasi{" "}
              {chartType === "trend"
                ? "tren harga"
                : chartType === "radar"
                ? "profil klaster"
            ...
            <div className="text-sm text-gray-500">
              Pilih jenis grafik di atas dan klik "Mulai Analisis"
            </div>
          </div>
        </div>
      )}

      {/* Chart Insights - Show only when data exists */}
      {hasData && (
        <div
            ...
            <div>
              ...
              >
                {chartType === "radar" ? (
                  <>
                    <p>
                      • <strong>Setiap Sumbu:</strong> Mewakili satu komoditas
                      dengan nilai ter-normalisasi (0-100%)
                    </p>
                    <p>
                      • <strong>Nilai Tinggi:</strong> Menunjukkan klaster
                      dengan harga komoditas yang relatif tinggi
                    </p>
                    <p>
                      • <strong>Bentuk Polygon:</strong> Menunjukkan profil
                      harga unik setiap klaster untuk semua komoditas
                    </p>
                    <p>
                      • <strong>Perbandingan:</strong> Semakin luas area,
                      semakin dominan karakteristik harga klaster tersebut
                    </p>
                  </>
                ) : chartType === "boxplot" ? (
                  <>
                    <p>
                      • <strong>Box Plot:</strong> Menampilkan distribusi harga
                      dari semua data point per komoditas
                    </p>
                    <p>
                      • <strong>Setiap Box:</strong> Mewakili satu klaster
                      dengan warna yang konsisten
                    </p>
                    <p>
                      • <strong>Box menunjukkan:</strong> Q1, median (garis
                      tebal), dan Q3; whiskers menunjukkan min-max
                    </p>
                    <p>
                      • <strong>Outliers:</strong> Ditampilkan sebagai titik di
                      luar whiskers
                    </p>
                  </>
                ) : chartType === "heatmap" ? (
                  <>
                    <p>
                      • <strong>Warna Merah:</strong> Korelasi positif kuat
                      (harga cenderung bergerak searah)
                    </p>
                    <p>
                      • <strong>Warna Biru:</strong> Korelasi negatif (harga
                      bergerak berlawanan arah)
                    </p>
                    <p>
                      • <strong>Warna Putih:</strong> Tidak ada korelasi atau
                      korelasi lemah
                    </p>
                    <p>
                      • <strong>Nilai 1.00:</strong> Korelasi sempurna (diagonal
                      utama - komoditas dengan dirinya sendiri)
                    </p>
                  </>
                ) : chartType === "scatter" ? (
                  <>
                    <p>
                      • <strong>Titik-titik:</strong> Mewakili kota dalam ruang
                      2D yang direduksi dari data komoditas
                    </p>
                    <p>
                      • <strong>Warna:</strong> Menunjukkan keanggotaan klaster
                      berdasarkan karakteristik harga
                    </p>
                    <p>
                      • <strong>Panah Biplot:</strong> Menunjukkan kontribusi
                      komoditas terhadap komponen utama
                    </p>
                    <p>
                      • <strong>Zoom/Pan:</strong> Scroll untuk zoom, drag untuk
                      pan, reset untuk kembali ke tampilan awal
                    </p>
                  </>
                ) : chartType === "silhouette" ? (
                  <>
                    <p>
                      • <strong>Bar Chart:</strong> Menampilkan silhouette score
                      setiap kota, diurutkan dari tertinggi
                    </p>
                    <p>
                      • <strong>Warna Bar:</strong> Menunjukkan keanggotaan
                      klaster berdasarkan warna yang konsisten
                    </p>
                    <p>
                      • <strong>Garis Rata-rata:</strong> Garis merah
                      putus-putus menunjukkan overall silhouette score
                    </p>
                    {/* <p>
                                            • <strong>Interpretasi:</strong>{" "}
                                            Skor &gt;0.5 = baik, 0.0-0.5 =
                                            sedang, &lt;0.0 = buruk
                                        </p> */}
                  </>
                ) : (
                  <>
                    <p>
                      ...
                      {selectedCommodity.toLowerCase()} selama periode{" "}
                      {data?.years?.[0]} -{" "}
                      {data?.years?.[data?.years?.length - 1]}
                    </p>
                    <p>
                      • Setiap warna garis merepresentasikan klaster yang
                      berbeda berdasarkan karakteristik harga
                    </p>
                    <p>
                      • Pola divergen menunjukkan adanya perbedaan signifikan
                      antar wilayah clustering
                    </p>
                  </>
                )}
              </div>
